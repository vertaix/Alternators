...
# model.load_state_dict(torch.load('LDIP_D_MC_5mc_LSTM_2L_V7.pt', map_location=torch.device('cpu')))
''' training phase'''
...
torch.save(model.state_dict(), 'LDIP_D_MC_5mc_LSTM_2L_V7.pt')
...
#     for num_rep in range(max_numb_repret):
#         mask_imput = get_mask_forcasting(x.shape[0], 0)
#         eps_z = model.sigma_z * torch.randn(z.shape[0],z.shape[2]).to(device)
#         eps_x = 1 * model.sigma_x * torch.randn(x[0].shape).to(device)
#         z_hat = model(x, mask_imput, eps_x, eps_z, True)
#         z_hat = z_hat.detach().cpu().numpy().squeeze()[1:,ii, :]
#
