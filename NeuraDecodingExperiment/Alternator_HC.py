...
dataset = pickle.load(open("data/NeuralData/example_data_hc_pp.p", "rb"))
batch_size=3
''' data'''
...
model = LDIP_A(latent_dim=z_tr.shape[-1], obser_dim=x_tr.shape[-1], sigma_x=.1,alpha=.3, importance_sample_size=1, n_layers=2,
...
Numb_Epochs=100
warmup_epochs=5
model.load_state_dict(torch.load('LDIP_Att_HC_5mc_2L_V2.pt'))#,map_location=torch.device('cpu')))
...
torch.save(model.state_dict(), 'LDIP_Att_HC_5mc_2L_V2.pt')
...
from metrics import get_R2, get_rho, get_RMSE, get_MAE,get_pearsonr, get_vendi
max_numb_repret=5
...
#         mask_imput = get_mask_forcasting(x.shape[0], 0)
#         eps_z = model.sigma_z * torch.randn(z[0].shape).to(self.device)
#         eps_x = 1 * model.sigma_x * torch.randn(x[0].shape).to(self.device)
#         z_hat = model(x, mask_imput, eps_x, eps_z, True)
...
trj_samples = np.arange(z_test.shape[1])
for ii in trj_samples:
    f, axes = plt.subplots(2, 1, sharex=True, sharey=False)
    for num_rep in range(max_numb_repret):
        mask_imput = get_mask_forcasting(x.shape[0], 0)
        eps_z =  torch.randn(z[0].shape).to(device)
        eps_x = 1 * model.sigma_x * torch.randn(x[0].shape).to(device)
        z_hat = model(x, mask_imput, eps_x, eps_z, True)
        z_hat = z_hat.detach().cpu().numpy().squeeze()[1:,ii, :]
    ...
    # vendi_te.append(np.mean(np.abs(get_vendi(z_hat.squeeze()))))
...
print('test vendi-mean=%f, std=%f' % (np.mean(vendi_te), np.std(vendi_te)))